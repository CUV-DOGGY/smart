...
export function AddressPage() {
  ...
  const save = async (payload) => {
    if (editing?.address_id)
      await addressApi.update(editing.address_id, payload);
  ...
  return (
    <section className="page">
      ...
      {loading ? (
        <div className="empty-state">加载地址中…</div>
      ) : (
          ...
          {addresses.map((address) => (
            ...
            >
              <div className="card-top">
                ...
                {address.is_default && <span className="badge">默认</span>}
              </div>
              <p>
                {address.province}
              ...
              <footer>
                <button className="ghost" onClick={() => setEditing(address)}>
                  编辑
                </button>
                {!address.is_default && (
                  <button
