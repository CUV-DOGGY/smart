...
export function OrderPage() {
  ...
  const selectedItems = useMemo(
    () =>
      products
        .filter((item) => quantities[item.food_id] > 0)
        .map((item) => ({
          ...item,
          quantity: quantities[item.food_id],
        })),
    [products, quantities],
  );
  const goodsTotal = selectedItems.reduce(
  ...
  const changeQuantity = (foodId, delta, stock) => {
    setQuantities((current) => ({
      ...current,
      [foodId]: Math.max(0, Math.min(stock, (current[foodId] || 0) + delta)),
    }));
  };

  const createOrder = async () => {
    if (!addressId || !selectedItems.length) {
      setError('请选择收货地址和至少一件商品');
      return;
    }
    ...
    const payload = {
      ...
      items: selectedItems.map(({ food_id, quantity }) => ({
        food_id,
        quantity,
      })),
    };
    ...
    try {
      ...
      setNotice(
        `订单 ${result.order_id} 创建成功，总计 ¥${result.total_price.toFixed(2)}`,
      );
  ...
  const cancelOrder = async (id) => {
    ...
    try {
      ...
      await loadOrders();
      if (detail?.order_id === id) setDetail(await orderApi.get(id));
    } catch (requestError) {
      setError(requestError.message);
    }
  };
          ...
          {!shops.length ? (
            ...
            <>
                ...
                <label>
                  ...
                  >
                    ...
                    {addresses.map((address) => (
                      ...
                      >
                        {address.receiver_name} · {address.detail_address}
                      </option>
                    ))}
                  </select>
                </label>
                ...
                {products.map((product) => (
                  <article key={product.food_id}>
                    ...
                    <div className="quantity">
                      <button
                        onClick={() =>
                          changeQuantity(product.food_id, -1, product.stock)
                        }
                      >
                        −
                      </button>
                      <b>{quantities[product.food_id] || 0}</b>
                      <button
                        onClick={() =>
                          changeQuantity(product.food_id, 1, product.stock)
                        }
              ...
              <div className="checkout-summary">
                <div>
                  <span>商品金额</span>
                  <strong>¥{goodsTotal.toFixed(2)}</strong>
                </div>
                <div>
                  <span>配送费</span>
                  <strong>
                    ¥{(selectedShop?.delivery_fee || 0).toFixed(2)}
                  </strong>
                </div>
                <div className="total">
                  <span>预计合计</span>
                  <strong>
                    ¥
                    {(goodsTotal + (selectedShop?.delivery_fee || 0)).toFixed(
                      2,
                    )}
                  </strong>
                </div>
                <button
                  ...
                  disabled={submitting || !selectedItems.length || !addressId}
                  onClick={createOrder}
                >
                  {submitting ? '提交中…' : '提交订单'}
                </button>
        ...
        <section className="panel history-panel">
          ...
          <div className="order-list">
            {orders.map((order) => (
              <article
                key={order.order_id}
                onClick={() => showDetail(order.order_id)}
              >
                <div>
                  <strong>
                    {order.items
...
function OrderDetail({ order, onClose, onCancel }) {
  const cancellable = ['pending_payment', 'paid', 'preparing'].includes(
    order.order_status,
  );
    ...
    <div className="modal-backdrop">
      <section className="modal order-modal">
        ...
        <dl>
          ...
          <dt>状态</dt>
          <dd>{STATUS_LABELS[order.order_status] || order.order_status}</dd>
          <dt>店铺</dt>
          <dd>{order.shop_id}</dd>
          <dt>商品</dt>
          <dd>
